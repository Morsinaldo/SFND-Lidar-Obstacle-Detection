...
void showLidarTopview()
{
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it)
    {
        ...
        double minZ = -1.40; // minimum height in m
        if (zw > minZ)
        {
            float val = it->x; // distance in driving direction
            float maxVal = worldSize.height; // maximum driving distance in m
            int red = min(255, (int)(255 * abs((val - maxVal) / maxVal))); // red <-> far
            int green = min(255, (int)(255 * (1 - abs((val - maxVal) / maxVal)))); // green <-> near
