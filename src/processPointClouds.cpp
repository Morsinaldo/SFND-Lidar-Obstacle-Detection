...
// std::vector<std::__fs::filesystem::path> ProcessPointClouds<PointT>::streamPcd(std::string dataPath) // for Mac
{

    std::vector<boost::filesystem::path> paths(boost::filesystem::directory_iterator{dataPath}, boost::filesystem::directory_iterator{});
    // std::vector<std::__fs::filesystem::path> paths(std::__fs::filesystem::directory_iterator{dataPath}, std::__fs::filesystem::directory_iterator{}); // for Mac
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
              ProcessPointClouds<PointT>::RansacPlaneSegment(typename pcl::PointCloud<PointT>::Ptr cloud, int max_iterations, float distance_tolerance)
{
  ...
  Ransac<PointT> segRansac(max_iterations, distance_tolerance, num_points);

  // get inliers from local-RANSAC implementation rather than PCL implementation
  std::unordered_set<int> inliers_result = segRansac.Ransac3d(cloud);

  auto end_time = std::chrono::steady_clock::now();
  auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
  std::cout << "My RANSAC plane segmentation took " << elapsed_time.count() << " milliseconds" << std::endl;
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::EuclideanClustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    Cluster_KDTree<PointT> cluster_points(cloud->points.size(), clusterTolerance, minSize, maxSize);

    std::vector<typename pcl::PointCloud<PointT>::Ptr> clusters = cluster_points.euclidean_cluster(cloud);

    auto end_time = std::chrono::steady_clock::now();
    auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
    std::cout << "KDTree clustering took " << elapsed_time.count() << " milliseconds and found " << clusters.size() << " clusters" << std::endl;
